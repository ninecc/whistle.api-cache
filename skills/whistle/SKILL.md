---
name: whistle
description: 用于配置、生成、审查或排查 Whistle 规则与抓包问题，包括 HTTP/HTTPS/WebSocket 代理、本地映射、请求/响应改写、通配/正则、filters、Values、reqRules/resRules、reqScript/resScript、移动端抓包、HTTPS 证书、Tunnel/captureError、w2 CLI、UI 面板、storage 路径和插件运行态排查。创建、修改或发布 whistle.xxx 插件包、hook、whistleConfig、lack 脚手架时应使用 whistle-plugin skill。
---
...
**分流：** 普通代理规则、mock、映射、抓包和运行态排查用本 skill；插件包开发、hook 代码、`whistleConfig`、`lack watch`、`rulesServer` / `uiServer` 等实现用 `whistle-plugin`。
...
- 裸 IP/host operation 等价连接目标覆盖（近似 `host://`）；URL 转发必须写完整 `http://` / `https://`
...
- 多个 `includeFilter` 任一命中即可通过；多个 `excludeFilter` 任一命中即排除；不要把多个 include 当 AND
