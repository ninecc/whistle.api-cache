...
本文记录 `whistle.api-cache` 下一阶段存储层改造方案，并作为 2026-06-09 存储重构的实施依据。

结论：方案方向符合项目要求，但不能直接一步切到 SQLite。原因是 `better-sqlite3` 等 SQLite 方案通常会引入 native 依赖，必须先验证 Whistle 插件运行环境、打包路径和 Node ABI 兼容性。当前阶段应先完成存储边界重构、body 对象存储拆分和可编辑 active body 语义，再在配置开关后接入 SQLite。
...
本次重构先完成 SQLite 前置层，而不是贸然引入数据库依赖：

- 定义 `CacheStore` 接口，避免 `CacheEngine` 直接绑定文件索引实现。
- 抽出 `BodyObjectStore`，统一 body 文件的 key、路径校验和原子写入。
- 新录制 body 写入 `objects/original/<sha256>.body`。
- 编辑后的 active body 写入 `objects/editable/<entryId>.body`。
- `CacheEntry` 增加 `originalBody*`、`activeBody*`、`updatedAt` 字段。
- 回放、导出和命中逻辑读取 active body。
- 录制命中同一条目时，如果已有 editable active body，不覆盖用户编辑内容。
- 删除和过期清理改为引用感知删除，避免共享 original body 被误删。
- 暴露可编辑 body 和恢复原始 body 的 store/engine/UI CGI 边界，供后续 UI 使用。

这一步完成后，文件存储仍是默认实现，但数据模型已经与 SQLite 方案对齐。
...
`original/` 保存录制得到的原始响应内容，按 sha256 命名，默认不可变。`editable/` 保存用户编辑后的响应内容，按稳定业务 id 命名，允许覆盖式更新或版本化扩展。根目录下的 `objects/<legacyHash>.body` 仅用于兼容旧文件存储数据，新录制数据不再写入该路径。
...
在 SQLite 依赖验证前，`FileCacheStore` 继续负责 `cache-index.json`，但必须遵守新边界：

- 对外实现 `CacheStore` 接口。
- 内部通过 `BodyObjectStore` 读写 body。
- 读取旧索引时自动补齐 `originalBodyKey`、`activeBodyKey`、`activeBodyKind` 等字段。
- 旧条目的 body key 兼容为 `<bodyHash>.body`。
- 新录制条目的 original body key 使用 `original/<sha256>.body`。
- `updateActiveBody()` 写入 `editable/<entryId>.body`，并把 active 指针切到 editable。
- `restoreOriginalBody()` 只切换 active 指针，不删除 editable 文件。
- `putEntry()` 表示录制或导入，不能承担“用户编辑”语义。
- 删除单条、清理过期时只删除没有被其它条目引用的 active/original body。

这层重构不是 SQLite 的替代品，而是降低 SQLite 落地风险的适配层。后续 `SqliteCacheStore` 应复用同一套 `BodyObjectStore` 与 store contract 测试。
...
1. 创建 `cache.sqlite3.tmp` 并写入 `schema_meta.schema_version = 1`。
...
5. 完整校验条目数量、body 引用和关键索引后，将 `cache.sqlite3.tmp` 原子 rename 为 `cache.sqlite3`。
6. 写入 `cache-index.json.bak.<timestamp>` 或迁移标记文件，保留旧文件供降级和人工排查。

启动时的兼容策略：

- 如果 `cache.sqlite3` 存在且 schema 版本可识别，优先使用 SQLite。
- 如果只存在 `cache.sqlite3.tmp`，视为上次迁移失败，删除临时数据库后重新迁移。
...
- 半迁移状态：只认完整 `cache.sqlite3`，临时数据库可删除重建。
...
1. 修复当前 `FileCacheStore` 删除共享 body 的风险，并补回归测试。已完成。
2. 定义 `CacheStore` 接口，保持现有 `FileCacheStore` 对外行为不变。已完成。
3. 抽出 `BodyObjectStore`，让当前文件存储也复用原子 body 写入和路径校验逻辑。已完成。
4. 增加 active body 编辑 API，但暂不接 UI 编辑器。已完成 store、engine 和 CGI 边界。
5. 增加恢复原始 body、诊断事件和垃圾清理。已完成恢复边界和引用感知删除；诊断事件待补充。
6. 验证 `better-sqlite3` 在 Whistle 插件运行环境里的可用性，失败则暂缓 SQLite 依赖落地。
7. 增加 SQLite store 的最小实现、schema 版本表和幂等迁移逻辑。
8. 最后接入 UI 编辑器和自由代理能力。
