...
当 `originalReq` 的 `method` 有效而 `fullUrl/url` 为空时，`url` 会继续回退到可用的请求/fallback 来源。
...
- 若直接 body 未命中且为“缺失”，则优先尝试 `req.getReqSession()`，否则再尝试 `req.getSession()` 读取 session 上的 body；内部以 `getSessionReaderFromRequest` 统一该取值入口并保留优先级不变。
