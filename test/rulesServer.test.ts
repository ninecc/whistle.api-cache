...
test('rules server falls back req.body empty string when originalReq.body is null', async () => {
  const root = await mkdtemp(join(tmpdir(), 'whistle-cache-rules-server-empty-req-body-with-null-original-'));
  const options = { baseDir: root };
  clearRecentEvents();
